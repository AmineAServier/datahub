...
import com.github.servier.datahub.graph.util.FileTransferOptions
...
/**
  * The `graph` class provides `upload` and `download` methods
  * to transfer files to/from remote host via '''graph'''.
  *
  * @constructor Create a new `graph` by specifying their `options`.
  * @param options Options passed by the user on the
  *                '''DataFrameReader''' or '''DataFrameWriter''' API.
  * @since 0.1.0
  */
class Graph(options: FileTransferOptions) extends Logging {
  ...
  def uploadFile: io.Serializable = {
    ...
    file.get.code match {
      ...
      case _ => throw new RuntimeException(
        s"Could not downland the file. graph API responded HTTP code : ${file.get.code}"
      )
    }
    /*
    file.getOrElse(
